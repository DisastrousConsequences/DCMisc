...
function Timer()
{
	...
	while (C != None)
	{
			...
			|| (TeamGame(Level.Game) == None && C.Pawn.Owner != PlayerSpawner)))		// or just not me
		{
			...
			C.Pawn.TakeDamage(DamageDealt, Pawn, C.Pawn.Location, vect(0,0,0), class'DamTypeEnhLightningRod');
			...
			if (C != None && C.Pawn != None && MonsterController(C) != None && FriendlyMonsterController(C) == None && Pawn != None 
		     	  && C.Enemy != Pawn && FastTrace(Pawn.Location,C.Pawn.Location) && FRand() < 0.25 )
