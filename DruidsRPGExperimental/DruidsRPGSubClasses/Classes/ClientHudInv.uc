...
function CopyArray()
{
	...
	for (x = 0; x < HUDMut.InitialXPs.Length; x++)
	{
		if(HUDMut.InitialXPs[x].PlayerName != "")
		{
			...
			ClientReceiveXP(x, HUDMut.InitialXPs[x].PlayerName, HUDMut.InitialXPs[x].XPGained, HUDMut.InitialXPs[x].PlayerClass, HUDMut.InitialXPs[x].SubClass);
		}
		else
...
simulated function ClientReceiveXP(int index, string PlayerName, int XPGained, int PlayerClass, string SubClass)
{
	if(Level.NetMode != NM_DedicatedServer)
	{
		if (index >= 0)
