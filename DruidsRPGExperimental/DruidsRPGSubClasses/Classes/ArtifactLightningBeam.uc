...
function Activate()
{
	...
	{
		if (LastUsed  + (TimeBetweenShots*AdrenalineUsage) > Instigator.Level.TimeSeconds)
		{
		     ...
		     && VSize(HitPawn.Location - StartTrace) < MaxRange && HitPawn.Controller.bGodMode == False)
		{
            ...
            DamageToDo = max(HitPawn.HealthMax * MaxPercentHealthMax,MaxDamage);	// half the health or 100, whichever is the greater
