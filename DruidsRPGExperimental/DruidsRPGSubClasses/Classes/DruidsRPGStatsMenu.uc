...
{
	local int x, y, OldAbilityListIndex, OldAbilityListTop, curSubClasslevel;
	...
	if (GiveItems != None)
	{
		...
		if (curSubClass != "" && curSubClass != curClass.default.AbilityName)		// no class is always ok
		{
		...
		{
			GiveItems.ServerGetAbilities(curSubClass);
...
function UpdateAbilityList()
{
	...
	for (x = 0; x < StatsInv.AllAbilities.length; x++)
	{
		...
		{	// do not add classes or subclasses to this abilities list. Handle separately
			...
			MaxLevel = GiveItems.MaxCanBuy(curSubClass, StatsInv.AllAbilities[x]);	// MaxLevel==0 means this class & subclass can't buy
...
function bool ShowAbilityDesc(GUIComponent Sender)
{
	...
	if (GiveItems != None)
	{
		MaxL = GiveItems.MaxCanBuy(curSubClass, Ability);	// MaxLevel==0 means this subclass can't buy
	}
	if (curClass == None)
