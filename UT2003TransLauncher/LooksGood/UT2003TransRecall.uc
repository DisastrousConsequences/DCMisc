...
function Translocate()
{
	...
	for ( C=Level.ControllerList; C!=None; C=C.NextController )
		if ( (C.Pawn != None) && (C.Pawn != Instigator) )
		{
			...
			DiffZ = Diff.Z;
			Diff.Z = 0;
			if ( (Abs(DiffZ) < C.Pawn.CollisionHeight + 2 * (Instigator.CollisionHeight*1.1))
				&& (VSize(Diff) < C.Pawn.CollisionRadius + (Instigator.CollisionRadius*1.1) + 8) )
			{
